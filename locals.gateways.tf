locals {
  ...
  virtual_network_gateways_express_route = {
    for hub_network_key, hub_network_value in var.virtual_hubs : "${hub_network_key}-express-route" => {
...
locals {
  virtual_network_gateways_vpn = {
    for hub_network_key, hub_network_value in var.virtual_hubs : "${hub_network_key}-vpn" => {

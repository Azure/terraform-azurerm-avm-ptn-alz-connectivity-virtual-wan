locals {
  ...
  firewall_policies = { for virtual_hub_key, virtual_hub_value in var.virtual_hubs : virtual_hub_key => merge({
    ...
    dns = merge({
      servers       = local.private_dns_resolver_enabled[virtual_hub_key] ? [module.dns_resolver[virtual_hub_key].inbound_endpoint_ips["dns"]] : []
      proxy_enabled = local.private_dns_resolver_enabled[virtual_hub_key]
